...
- I saldi in app provengono **solo** dal blocco JSON **`light_saldi`** nel `*_light.enc` (file **nella stessa cartella** del `.enc` completo sul desktop, senza sottocartelle). Il desktop li calcola con `compute_light_saldi_snapshot` / `saldi_footer_amount_vectors` in `main_app.py` (stessi numeri del footer **Saldi**, su 5 righe: saldi assoluti; di cui, impegni futuri; disponibilità oggi = assoluti - impegni futuri; impegni per carte sul conto di riferimento; disponibilità assoluta = assoluti + impegni per carte; righe `credit_card`; totali non-carta). Se manca, l’app mostra «Dati mancanti» e non calcola saldi dai movimenti nel light. Dopo ogni salvataggio desktop il sidecar si rigenera. All’accesso iOS con file completo, `recomputeLightSaldiFromFullDb` allinea il light. Al salvataggio iOS (futuro), `applyNewRecordToLightSaldi` aggiorna i saldi base; le colonne CC derivate restano coerenti dopo sync/ricomputo completo.
